...
function ciniki_marketplaces_marketUpdate(&$ciniki) {
    ...
    $rc = ciniki_marketplaces_checkAccess($ciniki, $args['business_id'], 'ciniki.marketplaces.marketUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.marketplaces.market', $args['market_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
